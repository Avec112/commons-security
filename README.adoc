...
[source,java]
.Using `KeyGeneratorUtils` to generate a `4096` bit `KeyPair`
----
KeyPair keyPair = KeyGeneratorUtils.generateRsaKeyPair4096();
----
[source,java]
.Alternative
----
...
----
KeyPair keyPair = CryptoUtils.generateRsaKeyPair4096();
...
----
KeyPair ecKeys = KeyGeneratorUtils.generateEcKeyPair(EccCurve.SECP384R1);
----
...
----
KeyPair keyPair = KeyGeneratorUtils.generateRsaKeyPair3072();
...
.Using `CryptoUtils` (recommended for simple use cases)
----
KeyPair keyPair = KeyGeneratorUtils.generateRsaKeyPair2048();
...
`KeyStorageUtils` provides utilities for storing and loading cryptographic keys in standard formats. It supports RSA, EC (ECDSA/ECIES), Ed25519, and symmetric AES keys.
...
KeyStorageUtils.savePrivateKey(keyPair.getPrivate(), Path.of("private.pem"));
KeyStorageUtils.savePublicKey(keyPair.getPublic(), Path.of("public.pem"));

// Load keys from disk
PrivateKey privateKey = KeyStorageUtils.loadPrivateKey(Path.of("private.pem"), "RSA");
PublicKey publicKey = KeyStorageUtils.loadPublicKey(Path.of("public.pem"), "RSA");
----
...
// Save keys
KeyStorageUtils.savePrivateKey(keyPair.getPrivate(), Path.of("ec_private.pem"));
KeyStorageUtils.savePublicKey(keyPair.getPublic(), Path.of("ec_public.pem"));

// Load keys
PrivateKey privateKey = KeyStorageUtils.loadPrivateKey(Path.of("ec_private.pem"), "EC");
PublicKey publicKey = KeyStorageUtils.loadPublicKey(Path.of("ec_public.pem"), "EC");
----
...
// Save keys
KeyStorageUtils.savePrivateKey(keyPair.getPrivate(), Path.of("ed25519_private.pem"));
KeyStorageUtils.savePublicKey(keyPair.getPublic(), Path.of("ed25519_public.pem"));

// Load keys
PrivateKey privateKey = KeyStorageUtils.loadPrivateKey(Path.of("ed25519_private.pem"), "Ed25519");
PublicKey publicKey = KeyStorageUtils.loadPublicKey(Path.of("ed25519_public.pem"), "Ed25519");
----
...
KeyStorageUtils.saveAesKey(secretKey, Path.of("aes_key.txt"));

// Load key
SecretKey loadedKey = KeyStorageUtils.loadAesKey(Path.of("aes_key.txt"));
----
...
// Save encrypted
KeyStorageUtils.saveAesKeyEncrypted(secretKey, password, Path.of("aes_key_encrypted.txt"));

// Load encrypted
SecretKey loadedKey = KeyStorageUtils.loadAesKeyEncrypted(Path.of("aes_key_encrypted.txt"), password);
----
...
String pemPrivateKey = KeyStorageUtils.toPemFormat(keyPair.getPrivate().getEncoded(), "PRIVATE KEY");
String pemPublicKey = KeyStorageUtils.toPemFormat(keyPair.getPublic().getEncoded(), "PUBLIC KEY");

// Convert from PEM format
byte[] privateKeyBytes = KeyStorageUtils.fromPemFormat(pemPrivateKey);
byte[] publicKeyBytes = KeyStorageUtils.fromPemFormat(pemPublicKey);
----
...
String base64PublicKey = KeyStorageUtils.exportPublicKeyAsBase64(keyPair.getPublic());

// Import from Base64 string
PublicKey importedKey = KeyStorageUtils.importPublicKeyFromBase64(base64PublicKey, "RSA");
----
...
boolean isSufficient = KeyStorageUtils.isRsaKeySufficient(keyPair, 2048); // true
