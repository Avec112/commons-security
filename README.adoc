...
This project uses https://github.com/diffplug/spotless[Spotless] to keep Java source formatting consistent across IDEs and build environments.

The Maven build is the source of truth for formatting. Do not rely on local IDE formatting settings as the final authority.
...
`mvn verify` also runs the Spotless check, so formatting issues should be fixed before opening a pull request.

The Java formatter is https://github.com/palantir/palantir-java-format[Palantir Java Format], a deterministic formatter derived from Google Java Format.

For IntelliJ IDEA, the recommended setup is:

* Install the `Spotless Applier` plugin.
* Enable `Run spotless` under `Settings -> Tools -> Actions on Save`.
* Leave IntelliJ's own `Reformat code` and `Optimize imports` actions disabled on save.

This keeps the IDE aligned with the Maven configuration in `pom.xml`.
