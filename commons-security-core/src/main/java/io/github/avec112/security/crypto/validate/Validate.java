...
public class Validate {
    ...
    public static void all(RunValidation... validations) throws MultipleMissingArgumentsError {
        ...
        final List<Throwable> failures = Arrays.stream(validations)
                .map(runValidation -> {
        ...
        if (failures.size() > 1) {
            MultipleMissingArgumentsError multipleFailuresError =
                    new MultipleMissingArgumentsError("Validation errors", failures);
